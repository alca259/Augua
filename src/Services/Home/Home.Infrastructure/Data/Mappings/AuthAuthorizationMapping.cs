...
internal class AuthAuthorizationMapping : IEntityTypeConfiguration<AuthAuthorization>
{
    public void Configure(EntityTypeBuilder<AuthAuthorization> builder)
    {
        ...
        builder.HasOne(fk => fk.Application).WithMany(m => m.Authorizations).HasForeignKey(fk => fk.ApplicationId).OnDelete(DeleteBehavior.Cascade);

        builder.HasIndex(i => new { i.ApplicationId, i.Status, i.Subject, i.Type }).HasDatabaseName("IX_AuthAuthorization_ApplicationId_Status_Subject_Type").IsUnique(false);
    }
}
